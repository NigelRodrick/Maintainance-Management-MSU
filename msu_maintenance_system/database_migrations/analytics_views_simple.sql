...
SELECT 
    ...
    AVG(DATEDIFF(hour, jr.created_at, jr.completed_at)) as avg_resolution_time,
    SUM(CASE WHEN jr.status = 'completed' THEN 1 ELSE 0 END) * 100.0 / COUNT(jr.id) as efficiency_score,
...
SELECT 
    ...
    jr.priority,
    jr.status,
    jr.created_at as created_date,
    jr.completed_at,
    jr.department_id,
    d.name as department_name,
    DATEDIFF(day, jr.created_at, jr.completed_at) as resolution_days
...
SELECT 
    ...
    jr.created_at as created_date,
    jr.completed_at,
    DATEDIFF(hour, jr.created_at, jr.completed_at) as resolution_hours,
    CASE 
        WHEN jr.priority = 'urgent' AND DATEDIFF(hour, jr.created_at, jr.completed_at) <= 4 THEN 1
        WHEN jr.priority = 'high' AND DATEDIFF(hour, jr.created_at, jr.completed_at) <= 8 THEN 1
        WHEN jr.priority = 'medium' AND DATEDIFF(hour, jr.created_at, jr.completed_at) <= 24 THEN 1
        WHEN jr.priority = 'low' AND DATEDIFF(hour, jr.created_at, jr.completed_at) <= 72 THEN 1
        ELSE 0
    END as within_sla,
    CASE 
        WHEN jr.priority = 'urgent' AND DATEDIFF(hour, jr.created_at, jr.completed_at) > 4 THEN DATEDIFF(hour, jr.created_at, jr.completed_at) - 4
        WHEN jr.priority = 'high' AND DATEDIFF(hour, jr.created_at, jr.completed_at) > 8 THEN DATEDIFF(hour, jr.created_at, jr.completed_at) - 8
        WHEN jr.priority = 'medium' AND DATEDIFF(hour, jr.created_at, jr.completed_at) > 24 THEN DATEDIFF(hour, jr.created_at, jr.completed_at) - 24
        WHEN jr.priority = 'low' AND DATEDIFF(hour, jr.created_at, jr.completed_at) > 72 THEN DATEDIFF(hour, jr.created_at, jr.completed_at) - 72
        ELSE 0
